...
export default function Reports() {
  ...
  return (
    <div className="page-shell">
      <p className="eyebrow mb-2">Compliance workspace</p><h1 className="page-title mb-2">Reports</h1>
      <p className="muted mb-8">Generate IMO-compliant compliance and fuel intelligence reports</p>

      <div className="grid grid-cols-3 gap-6">
        {/* Report config */}
        <div className="col-span-2 space-y-6">
          <div className="card">
            <h2 className="font-semibold text-[#e5eeeb] mb-4">Report configuration</h2>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="border border-white/10 rounded-xl max-h-48 overflow-y-auto bg-[#0e1516]">
                  <div
                    className="flex items-center gap-2 px-3 py-2 border-b border-white/10 text-sm text-[#55d58a] cursor-pointer hover:bg-white/[0.04]"
                  ...
                  {vessels.map((v) => (
                    <label key={v.id} className="flex items-center gap-3 px-3 py-2 hover:bg-white/[0.04] cursor-pointer">
                      <input
                        type="checkbox"
                      ...
                      />
                      <span className="text-sm text-[#d8e2df]">{v.name}</span>
                      <span className="text-xs text-[#62706f] ml-auto">IMO {v.imo_number}</span>
                    </label>
                  ))}
                  {vessels.length === 0 && (
                    <div className="px-3 py-4 text-[#62706f] text-sm">No vessels registered</div>
                  )}
                </div>
                <p className="text-xs text-[#62706f] mt-1">{selectedVessels.length} vessel(s) selected</p>
              </div>
            </div>
        ...
        <div className="space-y-4">
          <div className="card bg-ocean-50 border-ocean-100">
            <FileText size={24} className="text-[#53c8d2] mb-3" />
            <h3 className="font-semibold text-[#e5eeeb] mb-2">CII annual report</h3>
            <p className="text-sm text-[#8d9b99]">
            ...
            <div className="mt-4 space-y-1 text-xs text-[#8d9b99]">
          ...
          <div className="card border-[#e7b86a]/20 bg-[#e7b86a]/[0.06]">
            <p className="text-sm text-[#e7b86a] font-medium">Phase 1 status</p>
            <p className="text-xs text-[#b99a61] mt-1">
